...
const usePageReservaStore = create((set, get) => ({
  ...
  fetchReservas: async (filters = {}) => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:8080/reserva/obtenerReservaPaginado?${queryString}`);
      ...
      set({
        reservasStore: [],        // En caso que tenga un error por parte del servidor dejo la lista vacia
      });
    }
  }, 
