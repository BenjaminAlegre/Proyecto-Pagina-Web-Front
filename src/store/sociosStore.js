...
export const sociosStore = create((set, get) => ({
  ...
  getSocios: async () => {
    const response = await axios("http://localhost:8080/socio/socios");
  ...
  deleteSocioSelectect: async (id) => {
    const response = await axios.put(
      `http://localhost:8080/socio/bajaSocio/${id}`
    );
  ...
  reactivarSocioSelectect: async (socio) => {
    ...
    const response = await axios.put(
      `http://localhost:8080/socio/editarSocio/${socioAEnviar.id}`,
      socioAEnviar
    );
    console.log(await response.data);
    get().getSocios();
  ...
  modifySocio: async (socio) => {
    ...
    const response = await axios.put(
      `http://localhost:8080/socio/editarSocio/${modifySocioSendToBack.id}`,
  ...
  setStatusSocios: async () => {
    try {
      /*
          const response = await axios.get("http://localhost:8080/socio/status");
  ...
  addSocio: async (socio) => {
    ...
    const response = await axios.post(
      "http://localhost:8080/socio/alta",
      newSocioSendToBack
    );
  ...
  listDepartament: () => {
    ...
    useEffect(() => {
      axios.get("http://localhost:8080/dpto/departamentos").then((response) => {
...
export const disableDepto = (id) => {
  // axios.put(`http://localhost:8080/dpto/bajaDepartamento/${id}`)
  //     .then((response) => {
  //         return response;
  //     });
  return new Promise((resolve, reject) => {
    axios
      .put(`http://localhost:8080/dpto/bajaDepartamento/${id}`)
      .then((response) => {
...
export const getSocioById = (id) => {
  ...
  try {
    const response = axios.get(
      `http://localhost:8080/socio/obtenerSocio/${id}`
    );
