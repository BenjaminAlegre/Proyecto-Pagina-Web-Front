...
const usePageSociosStore = create((set, get) => ({
  ...
  fetchSocios: async (filters = {}) => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:8080/socio/busquedaPaginada?${queryString}`);

      // const response = await axios.get("http://localhost:8080/socio/busquedaPaginada", {
      //   params: {
      //     pagina: filters.page || 1,
      //     tamanio: filters.itemsPerPage || 9,
      //     categoria: tipoSocio,
      //     activo : activoValue,
      //     ...otherFilters
      //   }
